...
void test_sentinel()
{
    ...
    rc = redis_okvm_mgr_get_slaves(&g_mgr, sentinel_ip, sentinel_port);
    ...
    rc = redis_okvm_mgr_get_master(&g_mgr, sentinel_ip, sentinel_port);
    ...
    rc = redis_okvm_mgr_get_replicas(&g_mgr, sentinel_ips, redis_okvm_mgr_get_slaves);
    QUEUE_FOREACH(qptr, &g_mgr.slaves_head){
        hi = QUEUE_DATA(qptr, struct redis_okvm_host_info, link);
        HIREDIS_OKVM_LOG_INFO("Slaves ip(%s) port(%d)", hi->ip, hi->port);
    }
    ...
    g_mgr.master.port = 0;
    rc = redis_okvm_mgr_get_replicas(&g_mgr, sentinel_ips, redis_okvm_mgr_get_master);
    HIREDIS_OKVM_LOG_INFO("Master ip(%s) port(%d)", g_mgr.master.ip, g_mgr.master.port);
}
